...
def main(testdir, phenodir, tile_id, year, timesteps, hlsimg,
    ...
    # Load the data
    x, valid_mask = utils.load_data(testdir, year, tile_id,
    ...
    for model in glob(os.path.join(modeldir, '%s*seed*.h5' % modelname)):
        ...
        for ba in range(int(math.ceil(y_pred.shape[0]/BATCHSIZE))):
            ...
            if timesteps < 3 and x_px_std.shape[-2] != timesteps:
                x_px_std = x_px_std[:,:timesteps]
                x_pa_std = x_pa_std[...,:timesteps*x_px_std.shape[-1]]

            # Debug check
            print('test data sizes')
            print(x_px_std.shape)
            print(x_pa_std.shape)
            print(delta_s_std.shape)
            print(delta_p_std.shape)
        ...
        fig.suptitle(model)
        ax1.set_title('%s Prediction' % modelname)
        ax1.imshow(class_to_rgb(y_pred_class))
        ax1.axis('off')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--year', type=int, default=2019, help='year of data to use for testing')
    ...
    # Model info
    parser.add_argument('--modelname', choices=['lstm_cnn_delta', 'cnn'], help='name of model to test')
